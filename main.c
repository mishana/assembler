...
int main(int argc, char **argv) {
    int i;
    const char *file_to_compile;
    bool pre_assembly_res, first_pass_res, second_pass_res;
    ...
    for (i = 1; i < argc; ++i) {
        ...
        pre_assembly_res = run_pre_assembly(file_to_compile);
        if (!pre_assembly_res) {
        ...
        first_pass_res = run_first_pass(file_to_compile, &symtab, &machine_codes, &memory_codes);
        if (!first_pass_res) {
        ...
        second_pass_res = run_second_pass(file_to_compile, symtab, machine_codes, memory_codes);
        if (!second_pass_res) {
